...
refers to their inputs explicitly:
  ```console
...
To accept multiple arguments, it is also possible to use *functions that return
functions*. In the following example, the outer function returns another
...
expression. Expressions can be nested with one another. Unlike a regular language,
there are no "statements" in Nix. Statements run one after the other.

A statement would imply multiple values. In contrast, in Nix we have a single
expression that evaluates from the bottom up.
...
viceversa. The conversion is not trivial, but you can use these functions to do
so.
...
Merging attrsets is not something that you will do commonly. A preferred approach
